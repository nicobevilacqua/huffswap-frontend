...
function reducer(
  state: BalancesState,
  { type, payload }: { type: Action; payload: any }
) {
  switch (type) {
          ...
          [address]: {
            ...
            [tokenAddress]: uninitialized
              ...
              : {
                  ...
                  listenerCount:
                    state[chainId][address][tokenAddress].listenerCount + 1,
                },
          },
        },
      };
        ...
        [chainId]: {
          ...
          [address]: {
            ...
            ...tokenAddresses.reduce(
              (accumulator: any, tokenAddress: string, i: number) => {
    ...
    case Action.BATCH_UPDATE_EXCHANGES: {
      ...
      return {
        ...
        [chainId]: {
          ...state?.[chainId],
          ...exchangeAddresses.reduce(
            (accumulator: any, exchangeAddress: string, i: number) => {
...
const BalancesContext = createContext<
  [BalancesState, { [k: string]: (...args: any) => void }]
>([{}, {}]);
...
export default function Provider({ children }: { children: ReactNode }) {
  ...
  const batchUpdateExchanges = useCallback(
    (chainId, exchangeAddresses, tokenAddresses, values, blockNumber) => {
      dispatch({
...
export function Updater() {
  ...
  const [
    state,
    { update, batchUpdateAccount, batchUpdateExchanges },
  ] = useBalancesContext();
    ...
    if (typeof chainId === "number" && typeof blockNumber === "number") {
      for (const address of Object.keys(debouncedState?.[chainId] ?? {})) {
        for (const tokenAddress of Object.keys(
          debouncedState?.[chainId][address]
        )) {
          const active =
            debouncedState[chainId][address][tokenAddress].listenerCount > 0;
          if (active) {
            const cachedFetchedAsOf =
              fetchedAsOfCache.current?.[chainId]?.[address]?.[tokenAddress];
            const fetchedAsOf =
              debouncedState[chainId][address][tokenAddress]?.blockNumber ??
              cachedFetchedAsOf;
  ...
  const allTokens = useMemo(() => Object.keys(allTokenDetails), [
    allTokenDetails,
  ]);
  useEffect(() => {
    if (
      ...
      Promise.all(
        allTokens
          .filter((tokenAddress) => {
            const hasValue = !!stateRef.current?.[chainId]?.[account]?.[
              tokenAddress
            ]?.value;
            const cachedFetchedAsOf =
              fetchedAsOfCache.current?.[chainId]?.[account]?.[tokenAddress];
            const fetchedAsOf =
              stateRef.current?.[chainId]?.[account]?.[tokenAddress]
                ?.blockNumber ?? cachedFetchedAsOf;
      ...
      Promise.all(
        allExchanges
          .filter(({ exchangeAddress, tokenAddress }) => {
            const hasValueToken = !!stateRef.current?.[chainId]?.[
              exchangeAddress
            ]?.[tokenAddress]?.value;
            const hasValueETH = !!stateRef.current?.[chainId]?.[
              exchangeAddress
            ]?.["ETH"]?.value;

            const cachedFetchedAsOfToken =
              fetchedAsOfCache.current?.[chainId]?.[exchangeAddress]?.[
                tokenAddress
              ];
            const cachedFetchedAsOfETH =
              fetchedAsOfCache.current?.[chainId]?.[exchangeAddress]?.["ETH"];

            const fetchedAsOfToken =
              stateRef.current?.[chainId]?.[exchangeAddress]?.[tokenAddress]
                ?.blockNumber ?? cachedFetchedAsOfToken;
            const fetchedAsOfETH =
              stateRef.current?.[chainId]?.[exchangeAddress]?.["ETH"]
                ?.blockNumber ?? cachedFetchedAsOfETH;

            // if there's no values, and they're not being fetched, we need to fetch!
            if (
              (!hasValueToken || !hasValueETH) &&
              (typeof cachedFetchedAsOfToken !== "number" ||
                typeof cachedFetchedAsOfETH !== "number")
            ) {
              return true;
              // else, if there are values, check if they's stale
            } else if (hasValueToken && hasValueETH) {
              const blocksElapsedSinceLastCheckToken =
                blockNumber - fetchedAsOfToken;
              const blocksElapsedSinceLastCheckETH =
                blockNumber - fetchedAsOfETH;
...
export function useAllBalances() {
  ...
  return useMemo(
    () => (typeof chainId === "number" ? state?.[chainId] ?? {} : {}),
    [chainId, state]
  );
}
...
export function useAddressBalance(
  address: string,
  tokenAddress: string
): ethers.utils.BigNumber | undefined | null {
  const { chainId } = useWeb3React();
  ...
  const value =
    typeof chainId === "number"
      ? state?.[chainId]?.[address]?.[tokenAddress]?.value
      : undefined;

  return useMemo(
    () =>
      typeof value === "string" ? ethers.utils.bigNumberify(value) : value,
    [value]
  );
}
...
const daiTokenAddress = process.env.REACT_APP_DAI_ADDRESS;
const daiExchangeAddress = process.env.REACT_APP_DAI_EXCHANGE_ADDRESS;
const usdcTokenAddress = process.env.REACT_APP_USDC_ADDRESS;
const usdcExchangeAddress = process.env.REACT_APP_USDC_EXCHANGE_ADDRESS;
export function useETHPriceInUSD() {
  const { chainId } = useWeb3React();

  let daiReserveETH = useAddressBalance(daiExchangeAddress!, "ETH");
  let daiReserveToken = useAddressBalance(
    daiExchangeAddress!,
    daiTokenAddress!
  );
  ...
  useEffect(() => {
    if (
      ...
      const stablecoinReserves = [daiReservesObject, usdcReservesObject];
